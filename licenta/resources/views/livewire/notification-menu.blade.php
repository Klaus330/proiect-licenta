<button 
    ...
    <div class="absolute top-full mt-4 right-0 shadow-xl bg-white w-96 flex flex-col justify-start items-start rounded" x-show="showNotificationMenu" @click.away="showNotificationMenu = false;" style="display: none;">
        ...
        <ul class="w-full px-3">
            @forelse ($unreadNotifications->take(4) as $index => $item)
                <li class="text-left py-2 @if($index>=1) border-t border-gray-300 @endif text-gray-800 text-sm flex justify-between items-center w-full">
                    <a href="{{$item->data['link']}}" class="text-ellipsis">
                        {{$item->data['message']}}
                    </a>
