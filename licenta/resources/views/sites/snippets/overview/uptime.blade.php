    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <div class="w-full bg-white shadow-xl rounded-lg">
            ...
            <div class="flex flex-col px-4 py-2">
                <div class="grid grid-cols-2">
                    <div class="flex items-start flex-col">
                        ...
                        <span class="font-semibold text-sm"> {{ $latestStats[0]->duration ?? '--' }}</span>
                    </div>
                    <div class="flex items-start flex-col">
                        <span class="text-gray-400 text-sm font-light">Uptime</span>
            ...
            <div class="flex justify-content-between w-full rounded max-h-9">
                <div class="pr-1 flex-1 grid-cols-12 gap-1 flex">
                    @for ($i = 30; $i > 0 ; $i--)
                        @if(!isset($latestStats[$i]))
                            <div class="w-full h-full bg-gray-200"></div>    
                        @elseif($latestStats[$i]->successful())
                            <div class="w-full h-full bg-green-300 rounded-bl"  title="{{$latestStats[$i]->created_at->toFormattedDateString()}}"></div>
                        @endif
                    @endfor
                </div>
